...
9. Deliver via the configured channel:
   - **Slack channel** (default): `slack_send_message` to the configured channel (default `#general`, customer-configurable) with the full digest as the message body. Founder reads it Monday morning; teammates see the same signal.
   - **Slack DM**: `slack_send_message` to the founder's user ID if a customer prefers private.
...
The `/schedule` prompt template lives in the README. The placeholders (`<YOUR_WORKSPACE_NAME>`, `<DEALS_LIST_ID>`, `<THRESHOLD>`, `<SLACK_CHANNEL>`) get filled in once when the schedule is created.
